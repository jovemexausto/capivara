...
## libkrun — `capivara/macos-gfxstream-vulkan` (12 commits)
...
| 0010 | `virtio/gpu: mitigate deferred TransferFromHost3d vs. dispatch-thread races` | Adds a `deferred_guard` module tracking in-flight deferred reads by resource id; `wait_idle(resource_id)` serializes same-resource deferred reads; `wait_idle_all()` makes every command except `TransferToHost3d` wait for all in-flight deferred reads to drain. | Real, observed `SIGSEGV` in `VirtioGpuResource::TransferWithIov` from two distinct races around the deferred `TransferFromHost3d` thread (0007): same-resource reads running concurrently, and any dispatch-thread command racing a deferred read's lock-free `mResources` lookup. The `wait_idle(resource_id)` half is correct and kept; `wait_idle_all()` was a wrong fix (it deadlocked, and the crash it targeted was actually an unrelated dangling display pointer) — **superseded by 0011**, which removes it. |
| 0011 | `virtio/gpu: scope deferred-read guard per-resource, drop the global drain` | Removes `wait_idle_all()` and the "every non-`TransferToHost3d` command drains all in-flight reads" gate; replaces it with a per-resource `wait_idle()` on the resource-lifecycle commands only (attach/detach/unref/create2d/create3d/createBlob). | The global drain deadlocked: it also blocked `CmdSubmit3d`/ASG context-ping commands the host's ASG render thread needs serviced to drain the guest's Vulkan command ring — and a deferred read can itself be waiting on that ring (RenderEngine stalled in `ring_buffer_available_read`/`commitBuffer`/`vkFreeMemory`). The map-level race 0010's global drain was meant to cover is now handled structurally on the gfxstream side (0013, `mResources` shared_ptr + mutex), and the `TransferWithIov` `SIGSEGV` was root-caused as a dangling `Box<HeadlessDisplay>` in `crates/capy` (fixed via `Box::leak`, monorepo `fe63c4a`), not a deferred-read race. **With this commit + the capy fix, boot reaches `sys.boot_completed=1`** (35s uptime, 0 watchdog kills). |
...
| 0012 | `virtio/gpu: drain deferred read on CtxDetachResource to fix pipe UAF` | Adds `CtxDetachResource` to the per-resource `wait_idle` drain (alongside attach/detach-backing/unref/create from 0011). | `CtxDetachResource` -> gfxstream `DetachFromContext` sets `mHostPipe = nullptr` and frees the context's `RenderThreadPipe`/`RenderChannel`. A deferred `TransferFromHost3d` read on that resource, running lock-free on the "gpu xfer" thread, was mid `pipe->TransferFromHost` and dereferenced the freed channel — host **SIGSEGV at 0x0** in `VirtioGpuRenderThreadPipe::TransferFromHost` during SurfaceFlinger/SystemUI composition. Root-caused from a macOS crash report (deep dive 2026-06-28). Pairs with gfxstream 0014. |

## gfxstream — `capivara/macos-gfxstream-build` (15 commits)
...
| 0014 | `host: hold a strong ref to the resource pipe across blocking transfers` | `ReadFromPipeToLinear`/`WriteToPipeFromLinear` take a local `shared_ptr` copy of `mHostPipe` and use it across the blocking `TransferFromHost`/`TransferToHost`, instead of dereferencing the member. | A deferred `TransferFromHost3d` read races a concurrent `CtxDetachResource` -> `DetachFromContext` (`mHostPipe = nullptr`), which frees the pipe out from under the in-flight transfer. The local copy keeps it alive for the call's duration. Also fixes `WriteToPipeFromLinear`, which copied to a local but then still dereferenced the member. gfxstream half of the UAF fix; libkrun 0012 is the dispatch-side companion. Root-caused from a macOS crash report (deep dive 2026-06-28). |
| 0015 | `host/vulkan: fall back to vkMapMemory for host-visible blobs on Metal` | In `vkGetBlobInternal`, skip the ExternalBlob export branch on `__APPLE__` + `ExternalMemory::Mode::Metal` and fall through to the existing `vkMapMemory` + `addMapping` path. | The branch previously hit `GFXSTREAM_FATAL("ExternalBlob feature is not supported with external memory metal")` — MoltenVK has no exportable external-memory handle under Metal. The guest requests host-visible blobs constantly during SurfaceFlinger/gralloc composition, so this aborted the host (`Abort trap: 6` in `vkGetBlobInternal`) shortly into UI bring-up. The `vkMapMemory` fallback exposes MoltenVK's own host-visible pointer the same way the SHM path does. Deep dive 2026-06-28. |
